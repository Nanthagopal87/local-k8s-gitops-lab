...
│   ├── projects/
│   ├── applications/      # Argo CD Application objects (argocd-demo, k8s-learning, traefik-config, applicationsets)
│   └── applicationsets/   # ApplicationSet definitions (argocd-demo.yaml: list generator, dev + prod)
│
└── apps/
    └── demo-app/
        ├── base/
...
### Phase 7 — GitOps automation policy and the ApplicationSet Git generator

Done: controlled automated sync, self-heal and prune on one isolated sandbox, and Git-directory discovery of Applications. Details in the status table below.

---

### Later — Multi-Environment GitOps (originally listed here as Phase 7)
...
**Status:** Phase 7 complete. Argo CD now has nine Applications, and **automation is enabled on exactly one of them**: the isolated sandbox `phase7-sandbox` (automated sync + self-heal + prune, demonstrated with measurements). Every other Application stays on manual sync: the hand-made `argocd-demo`, `k8s-learning`, `traefik-config` and `applicationsets`; the Phase 6 generated `argocd-demo-dev` and `argocd-demo-prod`; and the two Applications `p7g-service-a-dev` and `p7g-service-b-dev` that a second ApplicationSet (`phase7-git-generator`, **Git directory generator**) discovered from folders in Git. The Phase 5 orphaned-ConfigMap question for the existing Applications is still an open decision (see [ADR-004](docs/decisions/ADR-004-kustomize-base-overlays.md)); the lab automation policy is in [ADR-006](docs/decisions/ADR-006-gitops-automation-policy.md).
...
| Phase 5 — Kustomize | Complete (base + `dev`/`prod` overlays for `argocd-demo`, rendered by Argo CD; environment switched with a single Application; see [docs/kubernetes/kustomize.md](docs/kubernetes/kustomize.md) and [ADR-004](docs/decisions/ADR-004-kustomize-base-overlays.md)). With prune off, each overlay switch leaves an orphaned generated ConfigMap and `argocd-demo` shows `OutOfSync` until it is removed; the one from this phase was cleaned up by an approved manual delete. |
| Phase 6 — ApplicationSets | Complete (ApplicationSet controller enabled, about 26 MiB; list generator generates `argocd-demo-dev` and `argocd-demo-prod` into separate namespaces with no ownership collision; all manual sync; see [docs/argocd/applicationsets.md](docs/argocd/applicationsets.md) and [ADR-005](docs/decisions/ADR-005-applicationsets.md)). The hand-made `argocd-demo` Application is unchanged. |
| Phase 7 — Automation policy + Git generator | Complete (automated sync, then self-heal, then prune demonstrated on the isolated `phase7-sandbox` only, with timings; a Git directory generator discovers `service-a` and `service-b` folders and generates manual-sync Applications, including add, remove and re-adopt experiments; no other Application changed; see [docs/argocd/automation.md](docs/argocd/automation.md), [docs/argocd/applicationsets.md](docs/argocd/applicationsets.md#16-the-git-directory-generator-phase-7) and [ADR-006](docs/decisions/ADR-006-gitops-automation-policy.md)). Automation for the existing Applications is **not** enabled and needs your explicit approval. |
...
> The cluster, the Kubernetes fundamentals, Argo CD and the adoption of the existing resources are done. GitOps (manual sync) now covers the demo app, the Phase 2 resources and the Traefik configuration; the fake Secret and the disposable demos stay outside it on purpose. Kustomize base + overlays are now proven on the demo app. ApplicationSets (list and Git directory generators) are now proven on top of it, and automated sync, self-heal and prune are proven on one isolated sandbox. Still open and separate: your decisions on automation for the existing Applications, on how orphaned generated ConfigMaps are handled for them, and on retiring the hand-made `argocd-demo` Application in favour of generated ones.
