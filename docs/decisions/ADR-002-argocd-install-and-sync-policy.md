...
4. **Sync policy: manual.** The Application deliberately has no `syncPolicy.automated`. Argo CD detects drift and Git changes and shows `OutOfSync`, but changes the cluster only when a sync is requested. Automated sync, self-heal and prune are deferred. *(Update, Phase 7: they were demonstrated on one isolated sandbox Application only; every other Application stays manual. See [ADR-006](ADR-006-gitops-automation-policy.md).)*
